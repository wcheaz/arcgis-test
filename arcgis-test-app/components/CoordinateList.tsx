...
const CoordinateList: React.FC<CoordinateListProps> = ({ title, points, center, userLocation, onPointClick, onPointDelete }) => {
    return (
        <div className="flex flex-col h-full bg-white dark:bg-zinc-900 shadow-md rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-800">
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 backdrop-blur-sm">
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
                    {points ? `${points.length + (userLocation ? 1 : 0)} points` : 'No points'}
                </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                ...
                {userLocation && center && (
                    <div
                        onClick={() => onPointClick && onPointClick(userLocation)}
                        className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md border border-blue-100 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors duration-200 group cursor-pointer relative"
                    >
                        <div className="flex items-center justify-between mb-1">
                            <span className="text-xs font-bold text-blue-500 dark:text-blue-400 group-hover:text-blue-600 transition-colors">
                                My Location
                            </span>
                            <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400">
                                {calculateDistance(center.latitude, center.longitude, userLocation.latitude, userLocation.longitude).toFixed(2)} mi
                            </span>
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-sm">
                            <div className="flex flex-col">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-400">Lat</span>
                                <span className="font-mono text-zinc-700 dark:text-zinc-300">{userLocation.latitude.toFixed(4)}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] uppercase tracking-wider text-zinc-400">Lon</span>
                                <span className="font-mono text-zinc-700 dark:text-zinc-300">{userLocation.longitude.toFixed(4)}</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {points && points.length > 0 ? (
                    points.map((point, index) => {
                        const distance = center
                            ? calculateDistance(center.latitude, center.longitude, point.latitude, point.longitude)
                            : null;
